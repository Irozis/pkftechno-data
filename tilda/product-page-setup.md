...
Для формы заявки подключите в Tilda сервис приёма данных «Email» и добавьте следующие адреса получателей:

- `pkf@pkftechno.ru`
- `nb@pkftechno.ru`
- `df@pkftechno.ru`

Каждый адрес необходимо подтвердить по ссылке из письма Tilda. После подтверждения отметьте сервис «Email» в настройках формы и переопубликуйте страницу. Форма передаёт в заявке заполненные поля, а также скрытые поля `product_title`, `product_slug`, `product_url` и `product_description`, поэтому в письме будет указано название товара и страница, с которой отправлена заявка.
